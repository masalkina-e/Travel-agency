...
function filterPrice(tours) {
  ...
  if (minPriceValue.length === 0 && maxPriceValue.length === 0) {
    ...
    const filteredPrice = tours.filter((tour) => {
      return (
        (tour.price >= minPriceValue && tour.price <= maxPriceValue) ||
        (tour.price >= minPriceValue && maxPriceValue.lenght === 0) ||
        (tour.price <= maxPriceValue && minPriceValue.lenght === 0)
      )
    })
...
async function init() {
  ...
  document
    ...
    .addEventListener("change", (event) => filterCountry(tours, event))
  document
    .getElementById("select-rating")
    .addEventListener("change", (event) => filterRating(tours, event))
  document
    .getElementById("select-duration")
    .addEventListener("change", (event) => filterDuration(tours, event))
  document
    .getElementById("btn-all-tours")
    .addEventListener("click", () => showAllTours(tours))
  document
    .getElementById("btn-close-min-max-price")
    .addEventListener("click", () => filterPrice(tours))
  document
    .getElementById("btn-favorite")
    .addEventListener("click", () => filterToFavorite(tours))
  document.getElementById("btn-send").addEventListener("click", makeBookTour)
  document
    .getElementById("btn-close-mw")
    .addEventListener("click", closeModalWindow)

  document.addEventListener("click", function (event) {
    const containerFilterPrice = document.getElementById(
      "container-filter-price"
    )
    const modalWindowFilterPrice = document.getElementById(
      "modal-window-min-max-price"
    )
